...
float _AWGN_generator(void) { // feels like overkill but fuck it
...
void _createBursts(bool *Bursts, unsigned input_data_length) {
	...
	for (unsigned int i = 1; i < 8*input_data_length; i++) {
		if (i % bits_per_cyc == 0) {
			...
			switch (chnl_st) {
			case GOOD_S:
				if (_randF() < P_a2) { chnl_st = BAD_UNS; }
				else { chnl_st = GOOD_S; }
				break;
			case BAD_S:
				if (_randF() < P_b2) { chnl_st = GOOD_UNS; }
				else { chnl_st = BAD_S; }
				break;
			case BAD_UNS:
				if (_randF() < P_b1) { chnl_st = GOOD_UNS; }
				else { chnl_st = BAD_S; }
				break;
			case GOOD_UNS:
				if (_randF() < P_a1) { chnl_st = BAD_UNS; }
				else { chnl_st = GOOD_S; }
				break;
			}
		}
