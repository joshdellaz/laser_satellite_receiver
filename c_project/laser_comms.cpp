...
extern "C" void applyLDPC(uint8_t *input)
{
    ...
    unsigned block_length = CODEWRD_L; // redundent? TODO
    ...
    else if (CODEWRD_R == (double)(2 / 3))
    {
        rate_index = 1;
    }
    else if (CODEWRD_R == (double)(3 / 4))
    {
        rate_index = 2;
    }
    else if (CODEWRD_R == (double)(4 / 5))
    {
        rate_index = 3;
    }
    else
    ...
    for (unsigned i_bit = 0; i_bit < CODEWRD_L; ++i_bit)
    { // converting bit stream to uint8_t array
